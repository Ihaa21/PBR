...
call glslangValidator -DFRAGMENT_SHADER=1 -S frag -e main -g -V -o %DataDir%\shader_brdf_convolution_frag.spv %CodeDir%\shader_brdf_convolution.cpp

call glslangValidator -DVERTEX_SHADER=1 -S vert -e main -g -V -o %DataDir%\shader_render_cubemap_vert.spv %CodeDir%\shader_render_cubemap.cpp
call glslangValidator -DFRAGMENT_SHADER=1 -S frag -e main -g -V -o %DataDir%\shader_render_cubemap_frag.spv %CodeDir%\shader_render_cubemap.cpp

call glslangValidator -DVERTEX_SHADER=1 -S vert -e main -g -V -o %DataDir%\pbr_vert.spv %CodeDir%\shader_model.cpp
call glslangValidator -DFRAGMENT_SHADER=1 -S frag -e main -g -V -o %DataDir%\pbr_frag.spv %CodeDir%\shader_model.cpp
...
cl %CommonCompilerFlags% %CodeDir%\pbr_demo.cpp -Fmpbr_demo.map -LD /link %CommonLinkerFlags% -incremental:no -opt:ref -PDB:pbr_demo_%random%.pdb -EXPORT:Init -EXPORT:Destroy -EXPORT:SwapChainChange -EXPORT:CodeReload -EXPORT:MainLoop
del lock.tmp
call cl %CommonCompilerFlags% -DDLL_NAME=pbr_demo -Fepbr_demo.exe %LibsDir%\framework_vulkan\win32_main.cpp -Fmpbr_demo.map /link %CommonLinkerFlags%
